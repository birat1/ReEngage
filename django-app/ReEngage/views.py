...
from .models import Admin, Avatar, Student, StudentAvatar
from .serializers import AdminSerializer, AvatarSerializer, ContactMessageSerializer, StudentSerializer
...
def update_login_streak(user: User) -> int:
    try:
        ...
        logger.exception(f"Error updating login streak: {str(e)}")
        return None
...
class apiStudent(viewsets.ModelViewSet):
    ...
    def get_item(self, user_id: int) -> Student | None:
        try:
    ...
    def create(self, request: HttpRequest) -> Response:
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                StudentAvatar.objects.create(student_id=student, avatar_id=default_avatar, is_equipped=True)
            except Avatar.DoesNotExist:
                logger.exception("Default avatar (id=1) does not exist")
                return Response({"error": "Default avatar not found"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            return Response(serializer.data, status=status.HTTP_201_CREATED)

        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def retrieve(self, request: HttpRequest, user_id: int) -> Response:
    ...
    @csrf_exempt
    def update(self, request: HttpRequest, user_id: int) -> JsonResponse:
    ...
    def destroy(self, request: HttpRequest, user_id: int) -> Response:
        student_instance = self.get_item(user_id)
        if not student_instance:
...
class apiAdmin(viewsets.ModelViewSet):
    ...
    def get_item(self, user_id: int) -> Admin | None:
        try:
    ...
    def update(self, request: HttpRequest, person_id: int) -> Response:
    ...
    def destroy(self, request: HttpRequest, user_id: int):
        admin_instance = self.get_item(user_id)
        if not admin_instance:
            return Response({"res": "Object with user_id does not exist"}, status=status.HTTP_400_BAD_REQUEST)
...
class apiAvatar(viewsets.ModelViewSet):
    ...
    def get_item(self, avatar_id: int) -> Avatar | None:
        try:
    ...
    def retrieve(self, request: HttpRequest, avatar_id: int) -> Response:
    ...
    def update(self, request: HttpRequest, avatar_id: int) -> Response:
        ...
        data = {"avatar_id": avatar_id, "name": request.data.get("name"), "price": request.data.get("price")}
...
def get_students(request: HttpRequest) -> Response:
    if not hasattr(request.user, "admin"):
    ...
    try:
        ...
        if hasattr(user, "admin"):
            ...
            student_data = {
                ...
                "english_percentage": (user.student.english_correct / user.student.english_answered * 100) if user.student.english_answered > 0 else 0,
                "maths_answered": user.student.maths_answered,
                "maths_percentage": (user.student.maths_correct / user.student.maths_answered * 100) if user.student.maths_answered > 0 else 0,
                ...
                "science_percentage": (user.student.science_correct / user.student.science_answered * 100) if user.student.science_answered > 0 else 0,
                "owned_avatars": [
                    {
        ...
        logger.exception(f"Error in get_user_info: {str(e)}")
        return Response({"error": str(e)}, status=500)
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def current_equipped_avatar(request: HttpRequest, user_id: int) -> Response:
    try:
        ...
        try:
            equipped_avatar = StudentAvatar.objects.get(student_id=user_id, is_equipped=True)
...
@api_view(["POST"])
def submit_contact_form(request: HttpRequest) -> Response:
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def purchase_avatar(request: HttpRequest) -> Response:
    user = request.user
    ...
    return Response(
        {"new_points": student.points, "purchased_avatar": {"avatar_id": avatar.avatar_id, "name": avatar.name, "price": avatar.price, "is_equipped": False}},
...
@api_view(["PATCH"])
@permission_classes([IsAuthenticated])
def equip_avatar(request: HttpRequest) -> Response:
    user = request.user
    if not hasattr(user, "student"):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def check_streak(request: HttpRequest) -> Response:
    user = request.user
    if not user.is_authenticated:
        return Response({"error": "Not authenticated"}, status=401)
