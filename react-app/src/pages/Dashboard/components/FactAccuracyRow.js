...
function FactAccuracyRow({
  ...
  const englishCorrect = parseFloat(english_correct);
  const englishAnswered = parseFloat(english_answered);
  const mathsCorrect = parseFloat(maths_correct);
  const mathsAnswered = parseFloat(maths_answered);
  const scienceCorrect = parseFloat(science_correct);
  const scienceAnswered = parseFloat(science_answered);

  const averageAccuracy =
    ((englishCorrect + scienceCorrect + mathsCorrect) /
      (englishAnswered + mathsAnswered + scienceAnswered)) *
    100;
      ...
      <div className="col-md-6 mb-3">
        <div className="bg-white shadow-sm p-4 rounded h-100">
          ...
          <div className="text-center">
            ...
            <p className="text-muted small mt-2">{getEncouragement()}</p>
          </div>
        </div>
      </div>
    </div>
